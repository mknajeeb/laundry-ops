...
def resolve_manual_cw_review(
    ...
    *,
    ...
    If the bag is still Disappeared-From-Portal qualified, refuse Return to
    Pending so soft disposition cannot hide a system DFP reason.
    """
    from backend.rinse_veewash_step1_api import _record_correction
    ...
    before = bulk_load_active_cw_overrides(cursor, organization_id, [bid]).get(bid)
    oi_id = (before or {}).get("order_instance_id") if before else None
    try:
        from backend.management_rinse_wf_review import _single_bag_dfp_qualified

        if _single_bag_dfp_qualified(
            cursor, organization_id, bid, order_instance_id=oi_id
        ):
            return {
                "ok": False,
                ...
                "bag_id": bid,
                "message": (
                    "Bag is still Disappeared From Portal qualified; "
                    "manual override alone cannot return it to Pending."
                ),
            }
    except Exception:
        pass
