"""PIN Hub consistency: Role visibility, current role, Inventory + R&C employee gates."""
...
def test_feature_defs_include_revenue_cost_and_inventory_labels():
    ids = [d["id"] for d in PIN_HUB_FEATURE_DEFS]
    assert "revenue_cost" in ids
    assert "inventory" in ids
    by_id = {d["id"]: d for d in PIN_HUB_FEATURE_DEFS}
    assert by_id["revenue_cost"]["label"] == "Revenue & Cost"
...
def test_hub_shows_inventory_and_revenue_when_employee_on():
    ...
    assert feats["revenue_cost"]["label"] == "Revenue & Cost"
