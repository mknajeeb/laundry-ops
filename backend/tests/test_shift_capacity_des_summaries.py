...
def test_thirty_minute_summary_matches_bag_timestamps():
    ...
    for row in result["time_summary"]:
        ...
        ready = sum(1 for b in bags if parse_clock_seconds(b["ready_to_fold"]) <= t)
        folded = sum(1 for b in bags if parse_clock_seconds(b["completed"]) <= t)
        assert row["ready_to_fold"] == ready
        assert row["folded"] == folded
        backlog = sum(
            1
            for b in bags
            if parse_clock_seconds(b["ready_to_fold"]) <= t and parse_clock_seconds(b["fold_start"]) > t
        )
...
def test_one_hour_summary_matches_bag_timestamps():
    ...
    assert all((row["time_sec"] - result["time_summary"][0]["time_sec"]) % 3600 == 0 for row in result["time_summary"])
    for row in result["time_summary"]:
        t = row["time_sec"]
        ready = sum(1 for b in result["bag_rows"] if parse_clock_seconds(b["ready_to_fold"]) <= t)
...
def test_summary_totals_reconcile_to_bag_detail():
    result = run_shift_capacity(_base())
    target = parse_clock_seconds("12:00 PM")
    bags_ready = sum(1 for b in result["bag_rows"] if parse_clock_seconds(b["ready_to_fold"]) <= target)
    bags_folded = sum(1 for b in result["bag_rows"] if parse_clock_seconds(b["completed"]) <= target)
...
def test_active_staffing_count_at_entry_boundaries():
    result = run_shift_capacity(_base())
    by_time = {row["time_sec"]: row for row in result["staffing_summary"]}
    t_folder = parse_clock_seconds("8:00 AM")
    assert by_time[t_folder]["active_folders"] >= 2
    assert by_time[parse_clock_seconds("7:00 AM")]["active_folders"] == 1
